...
| AD-05 | Free-tier cold starts must not repeatedly parse and embed the policy corpus, and production must not silently accept stale generated index state. | Build and validate the hidden policy index during the deploy build step, publish it in a fresh process, and require runtime readiness to validate the baked published index rather than rebuild it. | Production cold starts load a prepared index with no parse/embed/rebuild work; a missing or stale index degrades readiness and is repaired by a new explicit build/publish deployment cycle. |
...
| AD-S6-001 | The S6 agent prompt v1.0 required escalation for sensitive harassment, discrimination, and medical matters but did not state the frozen requirement to retrieve the workplace conduct policy explicitly. | Bump `PROMPT_VERSION` from `1.0` to `1.1` and require sensitive-topic handling to retrieve the Workplace Conduct Policy plus directly relevant evidence where tools are available, always escalate to HR / People and Culture, and never adjudicate whether an allegation, medical matter, or sensitive circumstance is true. | Aligns executable prompt behavior with the frozen S6 Agent Contract, preserves human review for sensitive matters, and makes later evaluation-score changes attributable to an explicit prompt version. |
...
### IMPLEMENTATION_SPEC.md v3.5 amendment — 2026-08-25

| Area | Old | New | Reason |
|---|---|---|---|
| Deployment index CLI | Deployment described `python -m rag.ingest` as the build-time indexing command. | Deployment uses the canonical process-separated `python -m rag.index build` followed by fresh-process `python -m rag.index publish`. | S8 implemented and verified the hidden-build and safe-publication lifecycle; the deployment specification must describe the production CLI that now owns that lifecycle. |
| Production startup | Startup compared index metadata and automatically performed a full ingest/rebuild when state was missing or stale. | Production runtime never parses, chunks, embeds, builds, or publishes the index; missing or stale state is repaired by a new explicit build/publish deployment cycle. | Runtime rebuilding duplicates expensive work, increases free-tier cold-start latency, mutates runtime state, and is unnecessary when deployment produces a validated build artifact. |
| Deployment verification | Build completion implied that a usable index would be available at runtime. | Deployment verifies the published collection contains exactly 400 chunks and that persisted metadata is current for corpus version and frozen embedding/chunk configuration. | S8 proved both `collection.count()` and `is_index_current()` through production store APIs; explicit verification fails closed before hosted runtime. |
| Runtime index health | The frozen design included `index:"building"` and a `/chat` warming-up path while startup rebuilt the index. | V1 runtime uses `ready` / `degraded`; there is no production runtime `building` state because runtime rebuilding is forbidden. | The build artifact is prepared before startup, so runtime should observe readiness rather than perform recovery work. |
| Local-development recovery | Local startup automatically rebuilt stale generated index state. | Corpus/configuration changes require explicit `python -m rag.index build` and `python -m rag.index publish`. | Explicit rebuilds preserve reproducibility and keep local and deployment index lifecycle semantics aligned. |
| AD-05 | Build-time ingestion was combined with an automatic local/fallback startup rebuild. | Build-time hidden-index construction + fresh-process publication is authoritative; runtime validates the baked index and never rebuilds it. | Preserves AD-05's cold-start objective while adopting the stronger process-separated lifecycle verified during S8. |
